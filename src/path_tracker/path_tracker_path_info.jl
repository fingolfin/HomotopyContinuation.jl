...
"""
function tracker_path_info(tracker::AdaptivePathTracker, x₀; debug::Bool = false, kwargs...)
    state = tracker.state
    ...
    init!(tracker, x₀; kwargs...)
    while tracker.state.code === :tracking
        push!(s, real(state.t))
...
path_table(info::TrackerPathInfo) = path_table(stdout, info)
function path_table(io::IO, info::TrackerPathInfo)
    header = [
        "",
        "s",
        "Δs",
...
function Base.show(io::IO, info::TrackerPathInfo)
    println(io, "TrackerPathInfo:")
